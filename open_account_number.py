...
def delete_today_data(config):
    ...
    try:
        with connection.cursor() as cursor:
            # 执行sql语句，插入记录
            sql = "DELETE FROM new_investor WHERE date = '%s'" %(present_date)
            cursor.execute(sql)
...
def get_new_investor():
    url = 'http://www.chinaclear.cn/cms-webapp/wcm/toErveyWeek_mzkb.action'
    ...
    for investors,new_personals,periods in zip(new_investor, new_personal_investor, period):
        ...
        try:
            with connection.cursor() as cursor:
            # 执行sql语句，插入记录
             sql = 'INSERT INTO new_investor (date, new_investor, new_personal, period) VALUES (%s, %s, %s, %s)'
